...
function Get-WPIEmployeeUsernames {
    ...
    $Search = Invoke-WPIBannerQuery "Select distinct CONCAT(USERNAME,'@WPI.EDU') as UserPrincipalName FROM $SaviyntTable WHERE PRIMARY_AFFILIATION = 'staff' or PRIMARY_AFFILIATION = 'affiliate' and USERNAME is not null and DEAD_IND = 'N' and IS_ACTIVE = '1' and EMPLOYEE_CLASS != 'RE'"
    $Search
}
...
function Get-WPIAdjunctFacultyUsernames {
    param (
        
    )
    $Search = Invoke-WPIBannerQuery "Select distinct CONCAT(USERNAME,'@WPI.EDU') as UserPrincipalName FROM $SaviyntTable WHERE PRIMARY_AFFILIATION = 'faculty' and Employee_class ='FA' and USERNAME is not null and DEAD_IND = 'N' and IS_ACTIVE = '1'"
    $search
}
...
function Get-BannerTotals {
    [CmdletBinding()]
    param()
    Write-Host "Number of Staff and Affiliate Users:" (Get-WPIEmployeeUsernames).count
    Write-Host "Number of Faculty Users:" (Get-WPIFacultyUsernames).count
    Write-Host "Number of Student Users:" (Get-WPIStudentUsernames).count
    Write-Host "Number of Alumni Users:" (Get-WPIAlumniUsernames).count
}