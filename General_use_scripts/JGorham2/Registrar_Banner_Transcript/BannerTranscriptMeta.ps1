...
Start-Transcript -Append -Path "$($logPath)\$($datestamp)_BannerUpload.log" -Force
...
foreach ($Entry in $BannerIndex) {
    $counter++
    Write-Progress -Activity "Processing Transcripts" -CurrentOperation $Entry.WPI_ID -PercentComplete (($counter / $BannerIndex.count) * 100)
    if ($entry.TRANSCRIPT_TYPE -eq "UNOFFICIAL") {
...
#shenanigans for sharepoint
# Set-PnPListItem -List SitePages -Identity "5" -Values @{"PageLayoutType"="Home"}
# that line gets rid of the title on a modern pade, the identity value is the ID of the page from Get-PnPListItem -List SitePages