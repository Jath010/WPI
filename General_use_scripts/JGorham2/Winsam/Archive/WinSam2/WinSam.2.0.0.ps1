...
#
# Version 2.0.RC.5
# Last Updated: 2011-02-08 12:24 - tcollins
#
...
function accountlookup($username){
    ...
    elseif ($enabled -ne 'True' -and $accountexpireddate -eq $null) {write-host 'This account (' $displayname '-' $userprincipalname ') is disabled (terminated).  There is no expiration date set for this account' -Foregroundcolor Black -BackgroundColor Red}
        ...
        if ($canonicalname -like 'admin.wpi.edu/Students/*') {
            write-host ''
        ...
        if ($accountexpireddate -ne $null -and $accountexpireddate -le $today) {
            write-host 'Account Expires On  : ' $accountexpireddate -Foregroundcolor Red
            }
        elseif ($accountexpireddate -ne $null -and $accountexpireddate -gt $today) {
            write-host ''
...
function GetFileshareMemberships($username){
    ...
    cls
    write-host '========================= WinSamaritan 2.0 ========================' -foregroundcolor Cyan
    write-host ''
    write-host 'Name                :' $displayname
    write-host 'Email               :' $userprincipalname
    write-host 'Title               :' $title
...
function emailforwarding ($username){
    ...
    if ($mailbox) {
        ...
        write-host 'Mail Delivery       : Mail is forwarded to Unix'
        }
    }
    else {
        write-host ''
        write-host 'Mail Delivery       : No Exchange mailbox exists for this user.' -ForegroundColor Red
        }
}
...
while ($anotheraccount -eq 'y') {
    ...
    if ($permissionsstatus -ne 'None'){
        while (!$username) {
            $username = read-host -Prompt 'Please enter a username for the ADMIN domain'
            $adminresult = isindomain $admindomain $username
            if ($adminresult -eq $null) {
                write-host ''
                Write-Host 'The account ' $username ' does not exist in the ADMIN domain.' -foregroundcolor Red
                write-host ''
    ...
    else {write-host 'Your account ' $env:username ' does not have permissions to run WinSamaritan 2.0' -ForegroundColor Red}
