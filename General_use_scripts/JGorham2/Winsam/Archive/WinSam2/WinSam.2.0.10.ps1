...
#
# Version 2.0.10
# Last Updated: 2012-10-15 - tcollins
#
...
function WinSam-Get-AccessLevel {
    ...
    $return = $null
    $usergroups = $null
    $arrdcs = (Get-ADDomainController -Filter {OperatingSystem -eq "Windows Server 2008 R2 Enterprise"} | Where {$_.Name -ne 'NEBULA'})
    $strDCHostname = $arrdcs[0].HostName.ToString()
    $usergroups = Get-ADPrincipalGroupMembership $username -Server $strDCHostname
...
function GetFileshareMemberships($username){
    ...
    cls
    write-host '======================= WinSamaritan 2.0.10 ========================' -foregroundcolor Cyan
    write-host ''
    write-host 'Name                :' $displayname
    write-host 'Email               :' $userprincipalname
    write-host 'Title               :' $title
