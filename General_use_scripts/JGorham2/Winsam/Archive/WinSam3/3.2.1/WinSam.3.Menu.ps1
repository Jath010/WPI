function WinSam-Menu-Main {
    ...
    Write-Host '     (2) Groups: AD Security and Distribution Groups'
    Write-Host '     (3) Groups: O365 Groups'
    Write-Host '     (4) Mailbox Information'
    Write-Host '     (5) Computer Information'
    Write-Host '     (6) Reset PIN'
    Write-Host '     (7) Search User by ID Number'
...
function WinSam-Menu-User {
    ...
    if ($UserAccessLevel -eq 'PasswordReset_lvl2' -or $UserAccessLevel -eq 'PasswordReset' -or $UserAccessLevel -eq 'SysAdmin') {
        while ($optionaction -eq $null) {
            ...
            #Write-Host "     (6) Manage Mailbox for $username"
            Write-Host ''
            ...
            switch ($option) { 
                ...
                #6 {$optionaction='ManageUserMailbox'} 
                'Q' {$optionaction='Exit'}
    ...
    switch ($optionaction) {
        ...
        'ManageUserMailbox' {WinSam-Menu-UserMailbox}
        'Exit' {exit}
        }
    }
...
function WinSam-Menu-UserMailbox{
    $optionaction = $null
    while ($optionaction -eq $null) {
        $alias = $Global:mailbox.Alias
        Clear-Host
        Write-Host (WinSam-Write-Header $HeaderTitle $MenuLength -Center) -ForegroundColor $Global:bcolor -BackgroundColor $Global:fcolor
        Write-Host (WinSam-Write-Header '' $MenuLength) -ForegroundColor $Global:bcolor -BackgroundColor $Global:fcolor
        Write-Host ''
	    ...
	    Write-Host ''
        Write-Host (WinSam-Write-Header 'Menu Options' $MenuLength $MenuIndent) -ForegroundColor Black -BackgroundColor Gray
        Write-Host '     (1) Look up another user mailbox'
        Write-Host '     (2) Recheck the current mailbox:' $alias
        Write-Host ''
        Write-Host '     (3) Forward: Set forward to Exchange'
        Write-Host '     (4) Forward: Set forward to UNIX'
        Write-Host '     (5) Forward: Set forward to manual location'
        Write-Host ''
        Write-Host '     (M) Main Menu' -ForegroundColor Green
        Write-Host '     (Q) Quit WinSamaritan' -ForegroundColor Red
        $option = read-Host 'Choose an option'
        ...
        switch ($option) { 
            ...
            2 {$optionaction='RepeatSearch'}
            3 {$optionaction='UserForwardExchange'}
            4 {$optionaction='UserForwardUNIX'}
            5 {$optionaction='UserForwardManual'}
            'M' {$optionaction='MainMenu'} 
            'Q' {$optionaction='Exit'}
            'exit' {$optionaction='Exit'}
            default {
    ...
    switch ($optionaction) {
        ...
        'RepeatSearch' {WinSam-Menu-UserMailbox}
        'MainMenu' {WinSam-Menu-Main}
        'UserForwardExchnage' {WinSam-Forward-Exchange $alias}
        'UserForwardUNIX' {WinSam-Forward-UNIX $alias}
        'UserForwardManual' {WinSam-Forward-Manual $alias}
        'Exit' {exit}
        }
    }
...
function WinSam-Menu-UnifiedGroup {
    $optionaction = $null
    while ($optionaction -eq $null) {
        Write-Host ''
        Write-Host ''
        Write-Host (WinSam-Write-Header 'Menu Options' $MenuLength $MenuIndent) -ForegroundColor Black -BackgroundColor Gray
        Write-Host '     (1) Look up another group'
        Write-Host '     (2) Recheck the current group:' $Global:UnifiedGroupAlias
        Write-Host ''
        Write-Host '     (M) Main Menu' -ForegroundColor Green
        Write-Host '     (Q) Quit WinSamaritan' -ForegroundColor Red
        $option = read-Host 'Choose an option'
    ...
    switch ($optionaction) {
        'AnotherGroup' {WinSam-UnifiedGroup}
        'RepeatSearch' {WinSam-UnifiedGroup $Global:UnifiedGroupAlias}
        'MainMenu' {WinSam-Menu-Main}
        'Exit' {exit}
        }
    }
