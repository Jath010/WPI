function Enable-Archiving {
    ...
    Get-Mailbox -Filter { ArchiveStatus -Eq "None" -AND RecipientTypeDetails -eq "UserMailbox" } | Enable-RemoteArchiving -Identity $_.alias
}


function Disable-Archiving {
    [CmdletBinding()]
    param (
...
function Enable-RemoteArchiving {
    [CmdletBinding()]
