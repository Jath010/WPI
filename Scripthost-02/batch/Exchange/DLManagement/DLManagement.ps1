...
$logPath        = "D:\wpi\Logs\DLManagement\Logs"
$logArchivePath = "D:\wpi\Logs\DLManagement\Logs-Archive"
...
elseif ($manualMode){
   $jsonPath = Read-Host "Full Path to JSON Folder"
}
else {
...
Function Archive-Logs ($DataPath, $ArchiveRoot, $Date) {
   ...
   Compress-Archive -LiteralPath $DataPath -DestinationPath $Archive_Zip
