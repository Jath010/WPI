function Sync-WPITermCourseGroups {
    [CmdletBinding()]
    param (
        $TargetTerm,
        $TargetYear
        ...
        foreach ($Course in $CourseData) {
            ...
            if ($Course.'SIS ID'.split("-")[1] -eq "Lab") {
                ...
                switch ($Term) {
                    ...
                    E { $Semester = "03"<# ; [string]$year = [int]$year+1 #> }
            ...
            if (($null -ne $TargetYear) -and ($targetYear -ne $year)) {
                continue
            }
            if (($null -ne $TargetTerm) -and ($TargetTerm -ne $term)) {
                continue
            }
            ...
            Write-host "Executing on $GroupName"
